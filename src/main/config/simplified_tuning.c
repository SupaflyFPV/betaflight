...
static void calculateNewGyroFilterValues(gyroConfig_t *gyroConfig)
{
    ...
    if (gyroConfig->gyro_lpf2_static_hz && gyroConfig->gyro_lpf2_static_hz <= SIMPLIFIED_GYRO_LPF2_MAX_HZ) {
        gyroConfig->gyro_lpf2_static_hz = constrain(GYRO_LPF2_HZ_DEFAULT * gyroConfig->simplified_gyro_filter_multiplier / 100, 0, GYRO_LPF2_STATIC_HZ_MAX);
    }
}
