        ...
        for (int i = 0; i < PRIMARY_CHANNEL_COUNT; i++) {
            if (i < THROTTLE) { // Throttle handled by smoothing rcCommand
                if (!smoothingData->filterInitialized) {
                    pt2FilterInit(&smoothingData->filter[i], pt2FilterGain(smoothingData->setpointCutoffFrequency, dT));
                } else {
                    pt2FilterUpdateCutoff(&smoothingData->filter[i], pt2FilterGain(smoothingData->setpointCutoffFrequency, dT));
                }
            } else {
                if (!smoothingData->filterInitialized) {
                    pt2FilterInit(&smoothingData->filter[i], pt2FilterGain(smoothingData->throttleCutoffFrequency, dT));
                } else {
                    pt2FilterUpdateCutoff(&smoothingData->filter[i], pt2FilterGain(smoothingData->throttleCutoffFrequency, dT));
                }
            }
        }
...
static FAST_CODE void processRcSmoothingFilter(void)
{
    ...
    for (int i = 0; i < PRIMARY_CHANNEL_COUNT; i++) {
        ...
        if (rcSmoothingData.filterInitialized) {
            *dst = pt2FilterApply(&rcSmoothingData.filter[i], rxDataToSmooth[i]);
        } else {
